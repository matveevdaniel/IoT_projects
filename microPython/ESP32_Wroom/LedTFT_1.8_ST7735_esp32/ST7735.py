...
class tft(object) :
  ...
  def __init__( self, aLoc, aDC, aReset ) :
    ...
    self.dc  = Pin(aDC, Pin.OUT_PP, Pin.PULL_DOWN)
    self.reset = Pin(aReset, Pin.OUT_PP, Pin.PULL_DOWN)
    rate = 200000 #100000000 #Set way high but will be clamped to a maximum in SPI constructor.
    cs = "X5" if aLoc == 1 else "Y5"
    self.cs = Pin(cs, Pin.OUT_PP, Pin.PULL_DOWN)
    self.cs.high()
    self.spi = SPI(aLoc, SPI.MASTER, baudrate = rate, polarity = 1, phase = 0, crc=None)
